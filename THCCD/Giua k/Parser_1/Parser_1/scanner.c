...
Token* readString(void) {
  ...
  token->string[0] = (char)currentChar;
  readChar(); // "
  while ((currentChar != EOF) && (charCodes[currentChar] == CHAR_DIGIT ||charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_SLASH || charCodes[currentChar] == CHAR_SPACE) && charCodes[currentChar] != CHAR_DOUBLEQUOTE) {
