...
void compileAssignSt(void) {
  ...
  eat(SB_ASSIGN);
  expresionType =  compileExpression();
  checkTypeEquality(lValueType,expresionType);
}
...
void compileForSt(void) {
  ...
  eat(SB_ASSIGN);
  expression1Type = compileExpression();
  checkBasicType(expression1Type);
  checkTypeEquality(var->varAttrs->type,expression1Type);

  eat(KW_TO);
  expression2Type = compileExpression();
  checkBasicType(expression2Type);

  checkTypeEquality(expression1Type,expression2Type);
...
void compileArgument(Object* param) {
  ...
  checkTypeEquality(argumentType,param->paramAttrs->type);
}
...
void compileCondition(void) {
  ...
  Type* expresionType1 = compileExpression();
  checkBasicType(expresionType1);
  switch (lookAhead->tokenType) {
  case SB_EQ:
    eat(SB_EQ);
    break;
  ...
  Type* expresionType2 = compileExpression();
  checkBasicType(expresionType2);
  checkTypeEquality(expresionType1,expresionType2);
}
