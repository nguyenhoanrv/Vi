...
Type* duplicateType(Type* type) {
  // TODO
  Type* typeElement = (Type*) malloc(sizeof(Type));
  typeElement->typeClass = type->typeClass;
  if(type->typeClass == TP_ARRAY){
    typeElement->arraySize = type->arraySize;
    typeElement->elementType = duplicateType(typeElement->elementType);
  }
  return typeElement;
...
ConstantValue* duplicateConstantValue(ConstantValue* v) {
  ...
  type->type = v->type;
  if(v->type == TP_INT)
    type->intValue = v->intValue;
  else if(v->type == TP_CHAR)
    type->intValue = v->charValue;
  return type;
}
